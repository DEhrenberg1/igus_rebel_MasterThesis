...
def main(args = None):
    ...
    action_noise = NormalActionNoise(mean=np.zeros(4,), sigma=0.8 * np.ones(4,))
    ...
    model = DDPG.load("exact_position_learner_gui_true_0_0", learning_starts = 0, action_noise = action_noise, gradient_steps = 5)
    model.set_env(env)
    #model = PPO("MultiInputPolicy", env=env, batch_size=3,n_epochs=2,n_steps=450)
    tmp_path = "~/igus_rebel_ws"
    # set up logger
    new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])  
    model.set_logger(new_logger)
    model.learn(total_timesteps = 50000, log_interval=10)
    #test 6 war 0.03 bei allen
    model.save("test_rb_ex_pos_learner_gui_true_0")
    model.save_replay_buffer("test_rb_ex_pos_learner_gui_true_0")
    
    # model = DDPG.load("test_rb_0")
    # model.load_replay_buffer("test_rb_0")
    # tmp_path = "~/igus_rebel_ws"
    # # set up logger
    # new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])  
    # model.set_logger(new_logger)
    # model.train(gradient_steps = 50000, batch_size= 32)
    # model.save("test_train_50k_32batch_0")

    # model = DDPG.load("test_train_50k_32batch_0")
    # model.set_env(env)
